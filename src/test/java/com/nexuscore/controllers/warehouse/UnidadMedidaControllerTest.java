...
import com.nexuscore.services.common.ConnectivityService;
import com.nexuscore.services.warehouse.UnidadMedidaService;
...
//         THEN: Verificamos el método save y "atrapamos" el argumento
    ...
    void testCreateError500() throws Exception {
        ...
        mockMvc.perform(post("/api/warehouse")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new UnidadMedidaModel(null, "Kilogramo", "KG"))))
                .andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.message").value("Ocurrió un error inesperado: Fallo en la base de datos")); // Mensaje corregido
    }
    ...
    void testBadRequestParameterType() throws Exception {
        mockMvc.perform(get("/api/warehouse/abc"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message", containsString("debe ser Long"))); // Ajustado a lo que genera ResponseFactory
    }
    ...
    void testHealthCheckUp() throws Exception {
        // Simulamos la respuesta del servicio de conectividad
        Map<String, Object> mockStatus = Map.of("status", "STABLE");
        when(connectivityService.checkFullConnectivity()).thenReturn(mockStatus);

        mockMvc.perform(get("/api/warehouse/health"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("success")); // ResponseFactory usa "success" para renderSuccess
    }
